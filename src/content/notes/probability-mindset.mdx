---
title: Probability Mindset
tags:
  - trading
  - probability
private: false
created: 2026-07-20
updated: 2026-07-20
---

A good bet is anything with a positive [[notes/expected-value|expected value]].

<BlockQuote work="Thinking in Bets" author="Annie Duke" medium="book">
What good poker players and good decision-makers have in common is their comfort with the world being an uncertain and unpredictable place. They understand that they can almost never know exactly how something will turn out. They embrace that uncertainty and, instead of focusing on being sure, they try to figure out how *unsure* they are, making their best guess at the chances that different outcomes will occur.
</BlockQuote>

<BlockQuote work="Thinking in Bets" author="Annie Duke" medium="book">
Thinking in bets starts with recognizing that there are exactly two things that determine how our lives turn out: the quality of our decisions and luck. Learning to recognize the difference between the two is what thinking in bets is all about.
</BlockQuote>

<BlockQuote work="Thinking in Bets" author="Annie Duke" medium="book">
Pete Carroll was a victim of our tendency to equate the quality of a decision with the quality of its outcome. Poker players have a word for this: “resulting.” When I started playing poker, more experienced players warned me about the dangers of resulting, cautioning me to resist the temptation to change my strategy just because a few hands didn’t turn out well in the short run.
</BlockQuote>

<BlockQuote work="Thinking in Bets" author="Annie Duke">
Being right feels really good. “I was right,” “I knew it,” “I told you so”—those are all things that we say, and they all feel very good to us. Should we be willing to give up the good feeling of “right” to get rid of the anguish of “wrong”? Yes.
</BlockQuote>

## Probability vs odds

Odds have their place, I assume, but [Eliezer Yudkowsky](https://www.lesswrong.com/w/probability-1#Centrality_of_the_classical_representation) says "classical probabilities are uniquely the quantities that must appear in the expected utilities to weigh how much we proportionally care about the uncertain consequences of our decisions."

## Further reading

- [[notes/probability|Probability]]
- *[[readwise/Books/thinking-in-bets|Thinking in Bets]]*, Duke
- *On the Edge*, Silver
- *The Signal and the Noise*, Silver
- *Thinking, Fast and Slow*, Kahneman
- *The Believing Brain*, Shermer
- *Kluge*, Marcus
- *Theory of Games and Economic Behavior*, von Neumann
- *Prisoner's Dilemma*, Poundstone
- [[notes/private/assets/neuroeconomics-camerer.pdf|"Neuroeconomics"]], Camerer